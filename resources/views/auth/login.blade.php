...
@section('body')
<div id="loginPage" class="min-h-screen bg-[#F4F7FA] flex items-center justify-center p-4 md:p-6">
    <div class="w-full max-w-5xl rounded-2xl overflow-hidden border border-[#B9DFFF] shadow-xl shadow-[#0879C9]/10 bg-white animate-fade-in">
        <div class="grid grid-cols-1 lg:grid-cols-5">
            <section class="lg:col-span-2 p-7 md:p-8 text-white relative overflow-hidden" style="background: linear-gradient(160deg, #0B82D3 0%, #0872BC 48%, #064A7C 100%);">
                ...
                <div class="absolute top-0 right-8 w-16 h-2 bg-[#F2CA00] skew-x-[-18deg]"></div>
                <div class="absolute bottom-0 left-8 w-24 h-2 bg-[#F2CA00] skew-x-[-18deg]"></div>
                <div class="relative z-10 h-full flex flex-col justify-between gap-8">
                    <div>
                        <div class="inline-flex items-center gap-3 mb-5">
                            <div class="w-10 h-10 rounded-lg bg-[#F2CA00] border border-[#FFE45C] text-[#075C99] shadow-lg shadow-black/10 flex items-center justify-center">
                                <i class="fas fa-qrcode text-base"></i>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking font-bold text-white/90">MAN 1 LAMPUNG SELATAN</p>
                                <p class="text-[11px] text-white/80">Madrasah Maju, Bermutu, Mendunia</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div class="rounded-lg border border-white/20 bg-white/10 px-3 py-2">
                            <div class="font-semibold">Sinkronisasi</div>
                            <div class="text-white/75 mt-0.5">Data Emis Kemenag</div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="lg:col-span-3 p-7 md:p-9 bg-white flex items-center">
                <div class="w-full max-w-md mx-auto">
                    <div class="mb-7">
                        <h2 class="text-xl md:text-2xl font-bold text-slate-800">Dashboard #Absensiswa</h2>
                        <p class="text-slate-500 text-sm mt-1.5">Masuk menggunakan akun masing-masing.</p>
                    </div>
                    ...
                    <div class="bg-slate-100 p-1.5 rounded-xl flex mb-6 border border-slate-200">
                        <button id="btnSiswaTab" onclick="switchLoginTab('siswa')" class="flex-1 py-2.5 text-sm font-bold rounded-lg shadow-sm bg-white text-indigo-700 ring-1 ring-black/5 transition-all duration-300 flex items-center justify-center gap-2">
                            <i class="fas fa-user-check"></i> Siswa|i
                        </button>
                        <button id="btnAdminTab" onclick="switchLoginTab('admin')" class="flex-1 py-2.5 text-sm font-medium rounded-lg text-slate-500 hover:text-slate-700 hover:bg-white/50 transition-all duration-300 flex items-center justify-center gap-2">
                            <i class="fas fa-chalkboard-teacher"></i> GTK | Admin
                        </button>
                    </div>
                ...
                <form id="loginForm" method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <div id="loginFormsWrap" class="min-h-[176px] relative">
                        ...
                        <div id="formAdminLogin" class="hidden space-y-4 animate-fade-in">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2 ml-1">Username|NIP</label>
                                <div class="relative group">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-600 transition-colors">
                                        <i class="far fas fa-chalkboard-teacher"></i>
                                    </div>
                                    <input type="text" id="username" name="username" class="block w-full pl-11 pr-4 py-3.5 bg-slate-50 border-slate-200 border rounded-xl text-slate-900 text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-slate-400" placeholder="Username|NIP" value="{{ old('username') }}">
                                </div>
                    ...
                    <button id="loginSubmitButton" type="submit" class="w-full py-3.5 px-6 bg-[#0879C9] hover:bg-[#0862A5] text-white font-bold rounded-xl shadow-md shadow-[#0879C9]/20 transition-all transform active:scale-[0.98] text-sm flex justify-center items-center group">
                        <span class="inline-flex items-center justify-center gap-2 leading-none">
                            ...
                            <span data-login-submit-label class="leading-none">Login</span>
                            ...
                            <form id="forgotPasswordForm" class="p-4 space-y-3" novalidate>
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2">
                                        <input id="forgotPasswordUsername" type="text" autocomplete="username" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5" placeholder="Username, email, atau NISN">
...
<script>
    ...
    function initLoginMotivation() {
        ...
        const motivations = [
            ...
            'Disiplin belajar dan hadir tepat waktu adalah kunci prestasi madrasah.',
            'Madrasah bukan hanya tempat belajar, tetapi tempat membangun masa depan.',
            ...
            'Datang ke madrasah tepat waktu adalah bentuk tanggung jawab pada diri sendiri.',
            ...
            'Langkah sederhana di madrasah hari ini dapat menjadi keberhasilan di masa depan.',
        ];
