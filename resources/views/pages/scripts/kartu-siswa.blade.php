<script>
    ...
    function scanFromFile(input) {
        ...
        Html5Qrcode.scanFile(input.files[0], true)
            ...
            .catch(() => {
                resDiv.innerHTML = `<div class="bg-red-50 text-red-600 p-3 rounded-xl border border-red-100 text-xs text-center">QR tidak terdeteksi. Coba foto lebih dekat.</div>`;
            });
        input.value = '';
    }
    ...
    function submitManualNisn() {
        ...
        if (!nisn) {
            showAlert('error', 'Masukkan NISN terlebih dahulu');
            return;
        }
                ...
                if (result.success) {
                    ...
                    if (cameraPopup && !cameraPopup.closed) {
                        try {
                            cameraPopup.postMessage({
                                type: 'SCAN_ERROR',
                                message: result.message || 'NISN tidak ditemukan'
                            }, '*');
                        } catch (e) {}
                    }
                }
    ...
    function highlightScanRow(tr, wrapper) {
        ...
        if (wrapperEl && tr) {
            wrapperEl.scrollTo({
                top: tr.offsetTop - (wrapperEl.clientHeight / 2) + (tr.offsetHeight / 2),
                behavior: 'smooth'
            });
        }
        ...
        setTimeout(async () => {
            try {
                ...
                for (let i = 0; i < dataToPrint.length; i++) {
                    ...
                    doc.setFontSize(5);
                    doc.text(nomorKartuCetak, x + 16.7, y + 39.7, {
                        align: 'center'
                    });
    ...
    window.loadGlobalConfig = function loadGlobalConfigPatched() {
        ...
        patchedRun('getAppConfig')
            .then((res) => {
                ...
                const f1 = document.getElementById('conf_masuk_mulai');
                if (f1) f1.value = cfg.jam_masuk_mulai || '06:00';
                const f2 = document.getElementById('conf_masuk_akhir');
                if (f2) f2.value = cfg.jam_masuk_akhir || '07:15';
                const f3 = document.getElementById('conf_pulang_mulai');
                if (f3) f3.value = cfg.jam_pulang_mulai || '15:00';
                const f4 = document.getElementById('conf_pulang_akhir');
                if (f4) f4.value = cfg.jam_pulang_akhir || '17:00';
            })
