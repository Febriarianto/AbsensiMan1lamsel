<script>
    ...
    const STAFF_LABEL_TITLE = STAFF_CONTEXT === 'piket' ? 'Guru | Piket' : 'Guru | Walikelas';
    const STAFF_KELAS_LABEL = STAFF_CONTEXT === 'piket' ? 'Kelas Tugas' : 'Guru';
...
function initDashboard() {
    ...
    if (role === 'wakel') displayRole = 'GURU';
    if (role === 'kamad') displayRole = 'KEPALA MADRASAH';
    if (role === 'waka') displayRole = 'WAKA';
...
function loadQRCodeSiswa() {
    ...
    function returnToDashboard() {
        if(role === 'admin' || role === 'waka' || role === 'kamad') loadAdminDashboard();
        else if(role === 'wakel') loadGuruDashboard();
        else loadSiswaDashboard();
    }
    ...
    function openGuruModal(guruData = null) {
        ...
        const title = modalNode?.querySelector('h3');
        if (title) title.textContent = isEdit ? 'Edit Akun Guru' : 'Tambah Akun';
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-3xl w-full">
            ...
            <div class="p-6 max-h-[75vh] overflow-y-auto">
                <form onsubmit="saveGuru(event, ${isEdit})" class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="${labelClass}">Username|NIP</label>
                            <input name="username" value="${guru?.username || ''}" placeholder="Username|NIP" required class="${inputClass}">
                        </div>
                        <div>
...
// ==========================================================================
    ...
    function handleFileImportGuru(input) {
        ...
        if (!window.XLSX) {
            showAlert('error', 'Fitur pembaca Excel belum siap. Muat ulang halaman lalu coba kembali.');
            input.value = '';
            return;
        }
        ...
        reader.onload = function(e) {
            try {
                ...
                if (!Array.isArray(rows) || rows.length < 2) {
                    hideLoading();
                    showAlert('error', 'File Excel kosong atau belum memiliki baris data.');
                    input.value = '';
                    return;
                }
                ...
                const aliases = {
                    ...
                    jenisKelamin: ['jenis kelamin', 'jenis_kelamin', 'jk'],
                    tanggalLahir: ['tanggal lahir', 'tanggal_lahir', 'tgl lahir'],
                    agama: ['agama'],
                    noHp: ['no hp', 'nohp', 'no handphone', 'hp', 'no_hp'],
                    alamat: ['alamat'],
                };
                const getValue = (row, keys) => {
                    const index = keys.map(key => headerMap.get(key)).find(index => index !== undefined);
                    return index === undefined ? '' : String(row[index] ?? '').trim();
                };

                if (!aliases.username.some(key => headerMap.has(key)) || !aliases.password.some(key => headerMap.has(key))) {
                    hideLoading();
                    showAlert('error', 'Format Excel salah. Header Username dan Password wajib tersedia.');
                    input.value = '';
                    return;
                }
                ...
                rows.slice(1).forEach((row, index) => {
                    ...
                    const item = {
                        username: getValue(row, aliases.username),
                        password: getValue(row, aliases.password),
                        name: getValue(row, aliases.name),
                        email: getValue(row, aliases.email),
                        kelas: getValue(row, aliases.kelas),
                        jenisKelamin: getValue(row, aliases.jenisKelamin),
                        tanggalLahir: getValue(row, aliases.tanggalLahir),
                        agama: getValue(row, aliases.agama),
                        noHp: getValue(row, aliases.noHp),
                        alamat: getValue(row, aliases.alamat),
                        _baris: index + 2,
                    };
                    if (item.username === '') errors.push(`Baris ${item._baris}: Username wajib diisi.`);
                    if (item.password === '') errors.push(`Baris ${item._baris}: Password wajib diisi.`);
                    formattedData.push(item);
                });

                if (formattedData.length === 0) {
                    hideLoading();
                    showAlert('error', 'Tidak ada data guru yang dapat diimport.');
                    input.value = '';
                    return;
                }
                ...
                if (errors.length > 0) {
                    hideLoading();
                    showAlert('error', `Data belum dapat diimport:\n${errors.slice(0, 8).join('\n')}`);
                    input.value = '';
                    return;
                }

                processImportGuru(formattedData, input);
            } catch (error) {
                hideLoading();
                showAlert('error', 'Format file tidak dapat dibaca. Gunakan template Excel yang disediakan.');
                input.value = '';
            }
        };
            ...
            .withSuccessHandler(res => {
                ...
                if (res.success) {
                    ...
                    const details = Array.isArray(res.errors) && res.errors.length > 0
                        ? `\n\nCatatan:\n${res.errors.join('\n')}`
                        : '';
                    const type = Number(res.added || 0) > 0 ? 'success' : 'error';
                    showAlert(type, `Import ${STAFF_LABEL_TITLE} Selesai! ${msg}${details}`);
                } else {
                    showAlert('error', res.message);
                }
            })
