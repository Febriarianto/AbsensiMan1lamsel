...
<head>
  ...
  <title>@yield('title', 'Dashboard') - {{ $appUiSettings['website_nama'] ?? 'Aplikasi #Absensiswa' }}</title>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    ...
    <script>
        ...
        window.APP_STUDENT_CARD_BACKGROUND_URL = @json($appUiSettings['student_card_background_url'] ?? '');
...
<body class="m-0 min-h-full overflow-x-hidden bg-[#F4F7FA] font-sans antialiased text-slate-800 selection:bg-indigo-600 selection:text-white">
  @yield('body')
  <script>
    (function () {
  ...
  <script>
    (function () {
      function formatRoleLabel(rawRole) {
        ...
        if (!role) return 'USER';
        if (role === 'wakel') return 'GURU';
        if (role === 'kepsek') return 'KEPALA MADRASAH';
        if (role === 'wakasek') return 'WAKA';
