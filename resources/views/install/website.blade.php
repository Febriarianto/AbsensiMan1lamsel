...
@section('title', 'Instalasi - Aplikasi')

@section('content')
<div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
    <div class="border-b border-gray-100 bg-gray-50/30 px-5 py-4">
        <h2 class="text-base font-bold text-gray-800">Langkah 3: Pengaturan Aplikasi</h2>
        <p class="mt-1 text-sm text-gray-500">Isi informasi aplikasi. Saat lanjut, sistem akan reset database (drop semua tabel), lalu jalankan migrate dan seeder otomatis.</p>
    </div>

    <form action="{{ route('install.website.store') }}" method="POST" class="space-y-4 p-5">
        @csrf
        <div>
            <label for="website_nama" class="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">Nama Aplikasi</label>
        ...
        <div>
            <label for="website_slogan" class="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">Nama Madrasah</label>
        ...
        <div>
            <label for="website_email" class="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">Email</label>
