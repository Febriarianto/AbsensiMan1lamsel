...
class AppUpdaterService
{
    ...
    public function installLatest(): array
    {
        ...
        try {
            ...
            $this->ensurePermanentEmisFeature();
    ...
    /**
     * Paket update boleh mengganti route dan sidebar utama, tetapi pengait menuju
     * modul EMIS lokal harus selalu dipasang kembali setelah proses penyalinan.
     */
    protected function ensurePermanentEmisFeature(): void
    {
        $routeFile = base_path('routes/web.php');
        $routeInclude = "require __DIR__ . '/emis.php';";

        if (!is_file($routeFile)) {
            throw new \RuntimeException('Route utama tidak ditemukan saat mempertahankan modul EMIS.');
        }

        $routeContents = (string) File::get($routeFile);
        if (!str_contains($routeContents, $routeInclude)) {
            File::append($routeFile, PHP_EOL . $routeInclude . PHP_EOL);
        }

        $sidebarFile = resource_path('views/partials/sidebar.blade.php');
        $menuInclude = "@include('partials.emis-sync-menu-item')";

        if (!is_file($sidebarFile)) {
            throw new \RuntimeException('Sidebar tidak ditemukan saat mempertahankan menu EMIS.');
        }

        $sidebarContents = (string) File::get($sidebarFile);
        if (str_contains($sidebarContents, $menuInclude)) {
            return;
        }

        $navClosePosition = strripos($sidebarContents, '</nav>');
        if ($navClosePosition === false) {
            throw new \RuntimeException('Struktur sidebar tidak valid saat mempertahankan menu EMIS.');
        }

        $injectedMenu = "        {$menuInclude}" . PHP_EOL;
        $sidebarContents = substr_replace($sidebarContents, $injectedMenu, $navClosePosition, 0);
        File::put($sidebarFile, $sidebarContents);
    }
