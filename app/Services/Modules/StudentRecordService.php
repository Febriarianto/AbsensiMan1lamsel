...
class StudentRecordService extends BaseActionService
{
    ...
    public function importSiswaBulk(array $args, $auth): array
    {
        ...
        if (!is_array($dataArray) || $dataArray === []) {
            return ['success' => false, 'message' => 'Tidak ada baris siswa yang dapat diimpor.'];
        }

        if (count($dataArray) > 2000) {
            return ['success' => false, 'message' => 'Maksimal 2.000 baris siswa untuk satu kali import.'];
        }
        ...
        foreach ($dataArray as $index => $item) {
            $rowNumber = is_array($item) && !empty($item['_baris']) ? (int) $item['_baris'] : $index + 2;
            if (!is_array($item)) {
                $skippedCount++;
                $errors[] = "Baris {$rowNumber}: format data tidak valid.";
                continue;
            }

            $nama = trim((string) ($item['nama'] ?? ''));
            $nisn = preg_replace('/[\s\']+/', '', (string) ($item['nisn'] ?? '')) ?? '';
            if ($nama === '' || $nisn === '') {
                $skippedCount++;
                $errors[] = "Baris {$rowNumber}: Nama dan NISN wajib diisi.";
                continue;
            }

            if (strlen($nisn) > 32) {
                $skippedCount++;
                $errors[] = "Baris {$rowNumber}: NISN melebihi 32 karakter.";
                continue;
            }
        ...
        try {
            ...
            Log::warning('Import siswa gagal diproses.', [
            ...
            return [
                'success' => false,
                'message' => 'Import gagal disimpan. Periksa kembali NISN, kelas, dan format data pada file.',
            ];
        }
