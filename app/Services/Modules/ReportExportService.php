...
class ReportExportService extends LessonWorkflowService
{
    ...
    public function downloadTemplate(Request $request)
    {
        ...
        if ($type === 'siswa') {
            $binary = $this->buildStudentImportTemplateXlsx($template[0] ?? []);
        ...
        if ($type === 'guru') {
            $binary = $this->buildGuruImportTemplateXlsx($template[0] ?? []);
    ...
    /**
     * Template guru memakai kolom teks untuk username, password, dan nomor HP.
     * Contoh data sengaja tidak disertakan agar tidak ikut terimport sebagai akun asli.
     */
    protected function buildGuruImportTemplateXlsx(array $headers): string
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $sheet->freezePane('A2');
        $sheet->setAutoFilter("A1:{$lastColumn}1");
        $sheet->getStyle('A2:B2001')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
        $sheet->getStyle('I2:I2001')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
        $sheet->getStyle("A1:{$lastColumn}2001")
            ->getBorders()
            ->getAllBorders()
            ->setBorderStyle(Border::BORDER_THIN);
        ...
        $guide = $spreadsheet->createSheet();
        $guide->setTitle('Petunjuk');
        $guide->setCellValue('A1', 'PETUNJUK IMPORT GURU');
        $guide->setCellValue('A3', '1. Isi data mulai baris kedua pada sheet Data Guru.');
        $guide->setCellValue('A4', '2. Username dan Password wajib diisi; username, password, dan No HP diformat sebagai teks.');
        $guide->setCellValue('A5', '3. Tanggal Lahir dapat ditulis YYYY-MM-DD atau DD/MM/YYYY.');
        $guide->setCellValue('A6', '4. Kelas bersifat opsional dan digunakan untuk menetapkan wali kelas.');
