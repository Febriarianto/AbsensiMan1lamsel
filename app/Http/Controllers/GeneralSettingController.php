...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request): RedirectResponse|JsonResponse
    {
        $validated = $request->validate([
            ...
            'student_card_background' => ['nullable', 'image', 'mimes:png,jpg,jpeg,webp', 'max:5120'],
            'remove_logo' => ['nullable', 'boolean'],
            'remove_favicon' => ['nullable', 'boolean'],
            'remove_student_card_background' => ['nullable', 'boolean'],
        ]);

        $settings = $this->getSettings();
        $logoPath = (string) ($settings['website_logo_path'] ?? '');
        $faviconPath = (string) ($settings['website_favicon_path'] ?? '');
        $studentCardBackgroundPath = (string) ($settings['student_card_background_path'] ?? '');
        ...
        if ($request->boolean('remove_student_card_background')) {
            $this->deletePublicFileIfExists($studentCardBackgroundPath);
            $studentCardBackgroundPath = '';
        }
        ...
        if ($request->hasFile('student_card_background')) {
            $this->deletePublicFileIfExists($studentCardBackgroundPath);
            $studentCardBackgroundPath = $request->file('student_card_background')->store('settings/student-cards', 'public');
        }
        ...
        $payload = [
            ...
            'student_card_background_path' => $studentCardBackgroundPath,
        ];
    ...
    protected function getSettings(): array
    {
        ...
        $settings['website_favicon_url'] = !empty($settings['website_favicon_path'])
            ...
            : null;
        $settings['student_card_background_url'] = !empty($settings['student_card_background_path'])
            ? Storage::disk('public')->url($settings['student_card_background_path'])
            : null;
