...
function handleTableSearch(type, query) {
        if (!tableState[type]) return;
        tableState[type].search = String(query ?? '').trim().toLocaleLowerCase('id-ID');
...
// FUNGSI BARU: Handle perubahan dropdown filter kelas
    function handleTableClassFilter(type, value) {
        if (tableState[type]) {
            tableState[type].classFilter = String(value ?? '').trim();
...
function handleTableStatusFilter(type, status) {
    ...
    function handleTableLimit(type, limit) {
        ...
        tableState[type].limit = limit === 'all'
            ? Infinity
    ...
    function changePage(type, direction) {
        ...
        if (!state) return;
        const maxPage = Number.isFinite(state.limit)
...
function processTableData(type) {
        ...
        let result = Array.isArray(state.fullData) ? [...state.fullData] : [];
        ...
        if ((type === 'siswa' || type === 'guru') && state.classFilter) {
            const selectedClass = String(state.classFilter).trim().toLocaleLowerCase('id-ID');
            result = result.filter((item) =>
                String(item?.kelas ?? '').trim().toLocaleLowerCase('id-ID') === selectedClass
            );
        }
        ...
        if (state.search) {
            const query = String(state.search).toLocaleLowerCase('id-ID');
            result = result.filter(item => 
                Object.values(item).some(val => 
                    String(val ?? '').toLocaleLowerCase('id-ID').includes(query)
                )
            );
        }
        ...
        const showAll = !Number.isFinite(state.limit);
        const totalPages = total === 0 ? 0 : (showAll ? 1 : Math.ceil(total / state.limit));
        ...
        const startIdx = showAll ? 0 : (state.page - 1) * state.limit;
        const endIdx = showAll ? total : startIdx + state.limit;
    ...
    // Inline handler pada Blade harus tetap dapat mengakses fungsi ketika script
    // halaman disajikan sebagai file JavaScript terpisah.
...
function initDashboard() {
    ...
    if (role === 'kepsek') displayRole = 'KEPALA MADRASAH';
    if (role === 'wakasek') displayRole = 'WAKIL KEPALA MADRASAH';
...
function patchedEscapeHtml(value) {
    ...
    function getSelectedSiswaRows() {
        ...
        return (tableState.siswa.fullData || []).filter((row) => selectedSiswaNisn.has(normalizeSiswaNisn(row?.nisn)));
    }

    function pruneSelectedSiswaRows() {
        const available = new Set(
            (tableState.siswa.fullData || [])
                .map((row) => normalizeSiswaNisn(row?.nisn))
                .filter(Boolean)
        );

        selectedSiswaNisn = new Set([...selectedSiswaNisn].filter((nisn) => available.has(nisn)));
    }
...
function renderSiswaRows(data, startIdx) {
        ...
        <tr class="hover:bg-gray-50 transition border-b border-gray-50 group">
            ...
            <td class="p-4 hidden xl:table-cell"><span class="bg-blue-50 text-blue-700 rounded text-xs font-bold">${patchedEscapeHtml(siswa.kelas || '-')}
            </span></td>
...
function loadQRCodeSiswa() {
    ...
    function showModal(content, maxWidthClass = 'max-w-2xl') {
...
function handleFileImportSiswa(input) {
        ...
        if (!window.XLSX) {
            showAlert('error', 'Library pembaca Excel belum siap. Muat ulang halaman lalu coba kembali.');
            input.value = '';
            return;
        }
        ...
        reader.onload = function(e) {
            try {
                ...
                if (headerIndex === -1) {
                    throw new Error('File Excel kosong. Isi data siswa mulai dari baris kedua.');
                }

                const headers = rows[headerIndex].map(normalizeHeader);
                const hasNama = headers.includes('nama') || headers.includes('nama lengkap');
                const hasNisn = headers.includes('nisn');
                if (!hasNama || !hasNisn) {
                    throw new Error('Header tidak sesuai. Gunakan template dan pastikan tersedia kolom Nama serta NISN.');
                }
                ...
                const aliases = {
                    ...
                    namaAyah: ['nama ayah'],
                    namaIbu: ['nama ibu'],
                    noHp: ['no handphone', 'no hp', 'hp'],
                    kelas: ['kelas'],
                    alamat: ['alamat'],
                    nomorKartu: ['nomor kartu', 'rfid', 'kode kartu'],
                };
                ...
                const jsonData = rows
                    ...
                    .map((row, index) => ({
                        nama: getCell(row, aliases.nama),
                        nisn: getCell(row, aliases.nisn).replace(/[\s']/g, ''),
                        jenisKelamin: getCell(row, aliases.jenisKelamin),
                        tanggalLahir: getCell(row, aliases.tanggalLahir),
                        agama: getCell(row, aliases.agama),
                        namaAyah: getCell(row, aliases.namaAyah),
                        namaIbu: getCell(row, aliases.namaIbu),
                        noHp: getCell(row, aliases.noHp),
                        kelas: getCell(row, aliases.kelas),
                        alamat: getCell(row, aliases.alamat),
                        nomorKartu: getCell(row, aliases.nomorKartu),
                        _baris: headerIndex + index + 2,
                    }));

                if (jsonData.length === 0) {
                    throw new Error('Belum ada data siswa. Isi data mulai dari baris kedua template.');
                }

                const invalidRows = jsonData.filter((row) => !row.nama || !row.nisn);
                if (invalidRows.length === jsonData.length) {
                    const rowNumbers = invalidRows.slice(0, 5).map((row) => row._baris).join(', ');
                    throw new Error(`Nama dan NISN wajib diisi. Periksa baris: ${rowNumbers}.`);
                }

                processImportSiswa(jsonData, input);
            } catch (error) {
                hideLoading();
                showAlert('error', error.message || 'File Excel tidak dapat diproses.');
                input.value = '';
            }
        };
...
function processImportSiswa(data, inputElement) {
        ...
        const formattedData = data.map(row => {
            ...
            return {
                ...
                nama: clean(row.nama || row['Nama Lengkap'] || row['Nama']),
                
                // NISN (Hapus kutip jika ada)
                nisn: clean(row.nisn || row['NISN']).replace(/[\s']/g, ''), 
                
                // Prioritas 1: Jenis Kelamin, Prioritas 2: JK
                jenisKelamin: clean(row.jenisKelamin || row['Jenis Kelamin'] || row['JK']), 
                
                // Prioritas 1: Tanggal Lahir, Prioritas 2: Tgl Lahir
                // Pastikan format di Excel Text (YYYY-MM-DD) atau Date
                tanggalLahir: clean(row.tanggalLahir || row['Tanggal Lahir'] || row['Tgl Lahir']), 
                
                agama: clean(row.agama || row['Agama']),
                
                namaAyah: clean(row.namaAyah || row['Nama Ayah']),
                
                namaIbu: clean(row.namaIbu || row['Nama Ibu']),
                
                // Prioritas 1: No Handphone, Prioritas 2: HP, Prioritas 3: No HP
                noHp: clean(row.noHp || row['No Handphone'] || row['HP'] || row['No HP']),
                
                kelas: clean(row.kelas || row['Kelas']),
                
                alamat: clean(row.alamat || row['Alamat']),

                nomorKartu: clean(row.nomorKartu || row['Nomor Kartu']),
                _baris: row._baris || null,
            };
        });
        ...
        actionRunner
            .withSuccessHandler(res => {
                ...
                if (res.success) {
                    tableState.siswa.fullData = [];
                    loadDataSiswa(); 
                    const msg = `Berhasil: ${res.added || 0}, Gagal/Duplikat: ${res.skipped || 0}`;
                    const details = Array.isArray(res.errors) && res.errors.length > 0
                        ? `\n${res.errors.slice(0, 3).map((item) => String(item)).join('\n')}`
                        : '';
                    if ((res.added || 0) === 0) {
                        showAlert('error', 'Tidak ada data baru yang diimpor. ' + msg + details);
                    } else {
                        showAlert('success', 'Import Selesai! ' + msg + details);
                    }
                } else {
                    showAlert('error', res.message);
                }
...
function downloadSingleKartuSiswa(nisn) {
    return downloadKartuSiswaBulk({ nisn });
}

window.downloadSelectedKartuSiswa = downloadSelectedKartuSiswa;
window.downloadSingleKartuSiswa = downloadSingleKartuSiswa;

async function downloadKartuSiswaBulk(options = {}) {
    // 1. Cek Filter & Data
    const selectedOnly = Boolean(options?.selectedOnly);
    const singleNisn = normalizeSiswaNisn(options?.nisn);
    ...
    if (singleNisn) {
        dataToPrint = allSiswa.filter((siswa) => normalizeSiswaNisn(siswa?.nisn) === singleNisn);
        const siswa = dataToPrint[0] || null;
        const fileIdentity = `${siswa?.nama || 'Siswa'}_${singleNisn}`
            .replace(/[\\/:*?"<>|]+/g, '_')
            .replace(/\s+/g, '_');
        fileName = `Kartu_${fileIdentity}.pdf`;
        confirmationText = `Mencetak kartu ${siswa?.nama || singleNisn}.`;
    ...
    setTimeout(async () => {
        try {
            ...
            const cardBackgroundUrl = String(window.APP_STUDENT_CARD_BACKGROUND_URL || '').trim();

            if (!cardBackgroundUrl) {
                throw new Error('Background kartu siswa belum diunggah. Buka Pengaturan > Umum lalu upload desain background kartu.');
            }

            const cardBackgroundDataUrl = await new Promise((resolve, reject) => {
                ...
                image.onerror = () => reject(new Error('Background kartu tidak dapat dimuat. Upload ulang gambar melalui Pengaturan Umum.'));
                image.src = cardBackgroundUrl;
            });
            ...
            for (let i = 0; i < dataToPrint.length; i++) {
                ...
                const noHpCetak = truncateText(pickValue(s, ['noHp', 'no_hp', 'phone', 'phone_number'], '-'), 18);
                const nomorKartuCetak = truncateText(pickValue(s, ['nomorKartu', 'nomor_kartu', 'cardNumber', 'card_number'], '----'), 14);

                // ==========================================
                // DESAIN KARTU
                // ==========================================

                // Background desain memenuhi seluruh kartu. Semua data di atasnya
                // tetap diambil dari database pada saat PDF dibuat.
                doc.addImage(
                ...
                // Panel kiri mengikuti format referensi: QR besar dan nomor kartu.
                const qrSize = 18.5;
                const qrX = x + 7.45;
                const qrY = y + 15.7;
                doc.setFillColor(248, 250, 252);
                doc.setDrawColor(226, 232, 240);
                doc.roundedRect(x + 3.2, y + 13.7, 27.0, 28.0, 2.2, 2.2, 'FD');
                ...
                doc.setFontSize(5);
                doc.text(nomorKartuCetak, x + 16.7, y + 39.7, { align: 'center' });

                // Panel kanan menutup contoh data yang mungkin tertanam di background.
                doc.setFillColor(255, 255, 255);
                doc.setDrawColor(241, 245, 249);
                doc.roundedRect(x + 31.7, y + 13.3, 51.7, 28.6, 1.5, 1.5, 'FD');

                // Nama menjadi judul, lalu detail dalam empat baris seperti referensi.
                doc.setTextColor(15, 23, 42);
                doc.setFont('helvetica', 'bold');
                doc.setFontSize(8.2);
                const nameMaxWidth = 48.0;
                const nameNaturalWidth = doc.getTextWidth(namaCetak);
                if (nameNaturalWidth > nameMaxWidth && nameNaturalWidth > 0) {
                    doc.setFontSize(Math.max(5.4, 8.2 * (nameMaxWidth / nameNaturalWidth)));
                }
                doc.text(namaCetak, x + 33.2, y + 17.2);

                const labelX = x + 33.2;
                const colonX = x + 49.2;
                const valueX = x + 51.0;
                const rows = [
                    ['NISN', cleanNisn, y + 22.9],
                    ['Jenis Kelamin', genderCetak, y + 28.1],
                    ['Tanggal Lahir', tanggalLahirCetak, y + 33.3],
                    ['No Hp', noHpCetak, y + 38.5],
                ];

                rows.forEach(([label, value, rowY]) => {
                    doc.setDrawColor(226, 232, 240);
                    doc.setLineWidth(0.12);
                    doc.line(labelX, rowY - 2.8, x + 81.8, rowY - 2.8);
                    doc.setTextColor(100, 116, 139);
                    doc.setFont('helvetica', 'normal');
                    doc.setFontSize(5.2);
                    ...
                    const maxValueWidth = cardWidth - (valueX - x) - 4;
                    const naturalWidth = doc.getTextWidth(printableValue);
                    if (naturalWidth > maxValueWidth && naturalWidth > 0) {
                        doc.setFontSize(Math.max(4.0, 5.2 * (maxValueWidth / naturalWidth)));
                    }
                    doc.text(printableValue, valueX, rowY);
                });
...
function handleArchiveAndReset() {
    ...
    function generateQRForSiswa(nisn, nama, kelas) {
        showModal(`
            <div data-card-preview-modal class="w-full overflow-hidden rounded-2xl border border-sky-100 bg-[#F4F7FA] shadow-2xl">
                <div class="flex items-center justify-between gap-4 bg-[#0879C9] px-5 py-4 text-white">
                    <div>
                        <h3 class="text-base font-extrabold sm:text-lg">Kartu Siswa</h3>
                        <p class="mt-0.5 text-xs text-sky-100">Preview background, data siswa, dan QR dari sistem</p>
                    </div>
                    <div class="flex shrink-0 items-center gap-2">
                        <button type="button"
                                data-print-student-card="${patchedEscapeHtml(nisn || '')}"
                                onclick="downloadSingleKartuSiswa(this.dataset.printStudentCard)"
                                class="inline-flex h-9 items-center gap-2 rounded-xl bg-[#F2CA00] px-3 text-xs font-extrabold text-slate-900 shadow-sm transition hover:bg-yellow-300 sm:px-4 sm:text-sm">
                            <i class="fas fa-print"></i>
                            <span>Cetak Kartu</span>
                        </button>
                        <button type="button" onclick="closeModal()"
                                class="flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 text-white transition hover:bg-white/25"
                                aria-label="Tutup preview kartu">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                </div>
                <div id="kartuSiswaContainer" class="max-h-[82vh] overflow-y-auto p-3 sm:p-5">
                    <div class="flex min-h-52 items-center justify-center rounded-xl bg-white text-sm font-semibold text-gray-500">
                        <i class="fas fa-circle-notch mr-2 animate-spin text-[#0879C9]"></i>
                        Memuat kartu siswa...
                    </div>
                </div>
            </div>
    ...
    window.loadQRCodeSiswa = async function loadQRCodeSiswaPatched(nisn, nama, kelas) {
        ...
        let data = {
            ...
            noHp: currentUser?.noHp || currentUser?.no_hp || '',
            nomorKartu: currentUser?.nomorKartu || currentUser?.nomor_kartu || ''
        };
            ...
            try {
                ...
                if (res.success && Array.isArray(res.data)) {
                    ...
                    if (match) {
                        ...
                        data.tanggalLahir = match.tanggalLahir || match.tanggal_lahir || data.tanggalLahir;
                        data.jenisKelamin = match.jenisKelamin || match.jenis_kelamin || data.jenisKelamin;
                        data.noHp = match.noHp || match.no_hp || data.noHp;
                        data.nomorKartu = match.nomorKartu || match.nomor_kartu || data.nomorKartu;
                    }
                }
            } catch (_) {}
        }
        ...
        const normalizePreviewGender = (rawValue) => {
            const value = String(rawValue || '').trim().toLowerCase();
            if (['l', 'lk', 'laki', 'laki-laki', 'pria', 'male'].includes(value)) return 'Laki-laki';
            if (['p', 'pr', 'perempuan', 'wanita', 'female'].includes(value)) return 'Perempuan';
            return rawValue || '-';
        };
        const formatPreviewDate = (rawValue) => {
            const value = String(rawValue || '').trim();
            if (!value) return '-';
            const parsed = new Date(/^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T00:00:00` : value);
            return Number.isNaN(parsed.getTime())
                ? value
                : parsed.toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' });
        };
        ...
        const cleanNisn = String(data.nisn).replace(/[^a-zA-Z0-9]/g, '').trim() || '0000';
        const backgroundUrl = String(window.APP_STUDENT_CARD_BACKGROUND_URL || '').trim();
        const qrId = `qrcode-kartu-siswa-${Date.now()}`;
        const namaPreview = truncatePreviewText(String(data.nama || '').toUpperCase(), 26);
        const tanggalLahirPreview = truncatePreviewText(formatPreviewDate(data.tanggalLahir), 12);
        const jenisKelaminPreview = truncatePreviewText(normalizePreviewGender(data.jenisKelamin), 12);
        const noHpPreview = truncatePreviewText(data.noHp, 18);
        const nomorKartuPreview = truncatePreviewText(data.nomorKartu || '----', 14);
        const isModalPreview = Boolean(container.closest('[data-card-preview-modal]'));

        if (!backgroundUrl) {
            container.innerHTML = `
                <div class="mx-auto max-w-2xl overflow-hidden rounded-2xl border border-amber-200 bg-white shadow-sm">
                    <div class="h-2 bg-gradient-to-r from-[#0879C9] via-[#0879C9] to-[#F2CA00]"></div>
                    <div class="p-8 text-center">
                        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-700">
                            <i class="fas fa-image text-2xl"></i>
                        </div>
                        <h3 class="mt-4 text-lg font-bold text-gray-900">Background kartu belum diunggah</h3>
                        <p class="mt-2 text-sm text-gray-600">Upload desain kartu melalui <strong>Pengaturan &gt; Umum</strong>. Setelah itu preview dan hasil cetak akan otomatis menggunakan desain yang sama.</p>
                    </div>
                </div>
            `;
            return;
        ...
        const previewHeading = isModalPreview ? '' : `
            <div class="mb-5 overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm">
                <div class="h-2 bg-gradient-to-r from-[#0879C9] via-[#0879C9] to-[#F2CA00]"></div>
                <div class="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 class="text-lg font-extrabold text-gray-900">Preview Kartu Siswa</h3>
                        <p class="text-sm text-gray-600">Tampilan ini sama dengan kartu yang akan dicetak.</p>
                    </div>
                    <span class="inline-flex w-fit items-center gap-2 rounded-full bg-sky-50 px-3 py-1 text-xs font-bold text-[#0879C9]">
                        <i class="fas fa-qrcode"></i>
                        QR dibuat oleh sistem
                    </span>
                </div>
            </div>
        `;
        ...
        container.innerHTML = `
            <div class="mx-auto w-full max-w-5xl">
                ...
                <div class="mx-auto w-full max-w-[860px]">
                    <div class="relative w-full overflow-hidden rounded-xl bg-white shadow-[0_18px_45px_rgba(15,23,42,0.18)] ring-1 ring-gray-200"
                         style="aspect-ratio: 86 / 54; container-type: inline-size;">
                        <img src="${escapePreviewHtml(backgroundUrl)}"
                             alt="Background kartu siswa"
                             class="absolute inset-0 h-full w-full object-fill">

                        <div class="absolute flex flex-col items-center border border-slate-200 bg-slate-50"
                             style="left:3.72%; top:25.35%; width:31.4%; height:51.85%; border-radius:8%; padding-top:2.2%;">
                            <div id="${qrId}" class="flex items-center justify-center bg-white"
                                 style="width:68.5%; aspect-ratio:1;"></div>
                            <div class="absolute bottom-[6%] w-full truncate px-[6%] text-center font-medium text-slate-500"
                                 style="font-size:clamp(6px,1.75cqw,15px);">${escapePreviewHtml(nomorKartuPreview)}</div>
                        </div>

                        <div class="absolute overflow-hidden bg-white/95"
                             style="left:36.86%; top:24.61%; width:60.12%; height:52.96%; border-radius:2.5%;">
                            <div class="absolute left-[3%] right-[3%] top-[4%] truncate font-extrabold text-slate-900"
                                 style="font-size:clamp(8px,3.05cqw,26px); line-height:1.12;">${escapePreviewHtml(namaPreview)}</div>

                            <div class="absolute inset-x-[3%] bottom-[3%] top-[22%] border-t border-slate-200">
                                <div class="grid h-1/4 items-center border-b border-slate-100"
                                     style="grid-template-columns:32% 4% 64%;">
                                    <span class="font-medium text-slate-500" style="font-size:clamp(6px,2.05cqw,17px);">NISN</span>
                                    <span class="font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">:</span>
                                    <span class="truncate font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">${escapePreviewHtml(cleanNisn)}</span>
                                </div>
                                <div class="grid h-1/4 items-center border-b border-slate-100"
                                     style="grid-template-columns:32% 4% 64%;">
                                    <span class="font-medium text-slate-500" style="font-size:clamp(6px,2.05cqw,17px);">Jenis Kelamin</span>
                                    <span class="font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">:</span>
                                    <span class="truncate font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">${escapePreviewHtml(jenisKelaminPreview)}</span>
                                </div>
                                <div class="grid h-1/4 items-center border-b border-slate-100"
                                     style="grid-template-columns:32% 4% 64%;">
                                    <span class="font-medium text-slate-500" style="font-size:clamp(6px,2.05cqw,17px);">Tanggal Lahir</span>
                                    <span class="font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">:</span>
                                    <span class="truncate font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">${escapePreviewHtml(tanggalLahirPreview)}</span>
                                </div>
                                <div class="grid h-1/4 items-center"
                                     style="grid-template-columns:32% 4% 64%;">
                                    <span class="font-medium text-slate-500" style="font-size:clamp(6px,2.05cqw,17px);">No Hp</span>
                                    <span class="font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">:</span>
                                    <span class="truncate font-bold text-slate-900" style="font-size:clamp(6px,2.15cqw,18px);">${escapePreviewHtml(noHpPreview)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <p class="mt-4 text-center text-xs text-gray-500">
                        QR berisi NISN <span class="font-mono font-bold text-gray-700">${escapePreviewHtml(cleanNisn)}</span> dan dapat digunakan untuk proses absensi.
                    </p>
                </div>
            </div>
        `;
        ...
        if (qrTarget) {
            ...
            const qrElement = qrTarget.querySelector('canvas, img');
            if (qrElement) {
